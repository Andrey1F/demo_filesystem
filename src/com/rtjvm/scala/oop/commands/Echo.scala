...
class Echo(args: Array[String]) extends Command {
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if (filename.contains(Directory.SEPARATOR))
      state.setMessage("Echo: filename with separators not supported")
    else {
      val newRoot: Directory = getRootAfterEcho(state.root, state.wd.getAllFoldersInThePath :+ filename, contents, append)
      if (newRoot == state.root)
        state.setMessage(filename + ": no such file")
      else
        State(newRoot, newRoot.findDescendant(state.wd.getAllFoldersInThePath))
    }
  }
